...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        if (key == null || value == null) {
            throw new IllegalArgumentException("Can't add void data.");
        }
        ...
        if (table[idx] == null) {
            ...
            ExternalChainingMapEntry<K, V> data = table[idx];
            while (data.getNext() != null && !data.getKey().equals(key)) {
                data = data.getNext();
            }
            if (data.getKey().equals(key)) {
                ...
                data.setNext(new ExternalChainingMapEntry<>(key, value));
            }
        }
        size++;
        return null;
    ...
    public V get(K key) {
        ...
        ExternalChainingMapEntry<K, V> data = table[Math.abs(key.hashCode() % table.length)];
        while (data != null && !data.getKey().equals(key)) {
            data = data.getNext();
        }
        if (data != null) {
    ...
    public Set<K> keySet() {
        ...
        for (ExternalChainingMapEntry<K, V> data : table) {
            if (data != null) {
    ...
    public void resizeBackingTable(int length) {
        ...
        ExternalChainingMapEntry<K, V>[] entries =
                (ExternalChainingMapEntry<K, V>[]) new ExternalChainingMapEntry[length];
        for (ExternalChainingMapEntry<K, V> data : table) {
            if (data != null) {
                int idx =
                        Math.abs(data.getKey().hashCode() % entries.length);
                entries[idx] = data;
            }
        }
        table = entries;
